  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Order failed: ${errorData.message || 'Database error occurred'}`);
      }
    } catch (err) {
      ...
      <main className="ml-64 flex-1 p-10">
        <header className="flex justify-between items-center mb-10">
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">
            Customer <span className="text-[#003d3d]">Portal</span>
          </h1>
        ...
        {activeTab === 'shop' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-4 md:grid-cols-10 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                <div key={p.id} className="bg-white rounded-[2rem] overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl transition-all group">
                  <div className="relative aspect-square overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4">
                      <span className="bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-black text-[#003d3d] uppercase tracking-widest">
                        {p.category}
                      </span>
                    </div>
                  </div>
                      ...
                      <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-8 py-5">
                          <button 
                            ...
                            className="flex items-center gap-2 bg-[#003d3d] text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-[#002d2d] transition-all shadow-md active:scale-95"
                          >
            ...
            <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in duration-200">
              ...
              <div className="flex gap-4 mb-6 p-4 bg-slate-50 rounded-2xl">
                ...
                <div>
                  ...
                  <p className="text-[#003d3d] font-black text-lg">₱{selectedProduct.price}</p>
                </div>
              </div>
