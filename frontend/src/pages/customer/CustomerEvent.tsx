...
const CustomerEvent = () => {
  ...
  const fetchEvents = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/events/customer/my-events', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Logic to switch between POST (new) and PUT (edit) for customer
    const url = editingEvent 
      ? `http://localhost:5000/api/events/customer/${editingEvent.id}` 
      : 'http://localhost:5000/api/events/customer';
